...
const TagAll = () => {
    ...
    const statusChangeHandler = (id) => {
        setLoading(true);
        axios
            .get(`${MAIN_URL}/api/tag/status/${id}`)
            .then((response) => {
    ...
    const deleteHandler = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .get(`${MAIN_URL}/api/tag/delete/${id}`)
                    .then((response) => {
                        if (response.data.code == 200) {
                            toast.success(response.data.message);
                        }
            ...
            selector: (row) => (
                <div className="">
                    <button className='mr-2'>
                        <Link to={`/dashboard/post/tag/edit/${row.id}`}>
        ...
        <>
            {
                isFetching ? (
                    ...
                    <DataTable
                        ...
                        actions={
                            <div className="">
                                <CustomButton text="+ Add new" path="/dashboard/post/tag/create" />
                            </div>
                        }
                        selectableRows
                        columns={columns}
