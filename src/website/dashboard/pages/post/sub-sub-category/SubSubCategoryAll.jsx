...
const SubSubCategoryAll = () => {
    ...
    const statusChangeHandler = (id) => {
        setLoading(true);
        axios
            .get(`${MAIN_URL}/api/subsubcategory/status/${id}`)
            .then((response) => {
    ...
    const deleteHandler = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .get(`${MAIN_URL}/api/subsubcategory/delete/${id}`)
                    .then((response) => {
                        if (response.data.code == 201) {
                            toast.success(response.data.message);
                        }
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row) => (
                <>
                    <div className="">
                        <p className="">{row.sub_category.name}</p>
                    </div>
                </>
            ),
        },
            ...
            selector: (row) => (
                <div className="">
                    <button className='mr-2'>
                        <Link to={`/dashboard/post/subsubcategory/edit/${row.id}`}>
        ...
        <>
            {
                isFetching ? (
                    ...
                    <DataTable
                        ...
                        actions={
                            <div className="">
                                <CustomButton text="+ Add new" path="/dashboard/post/subsubcategory/create" />
                            </div>
                        }
                        selectableRows
                        columns={columns}
...
export default SubSubCategoryAll;
